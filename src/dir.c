...
static int _task_dbmkdir(const char *taskdir, tman_arg_t * args)
{
...
static int _board_dbmkdir(const char *taskdir, tman_arg_t * args)
{
    char path[PATH_MAX + 1];
    sprintf(path, "%s/%s/%s/.tman", taskdir, args->project, args->board);
    return MKDIR(path);
}

static int _project_dbmkdir(const char *taskdir, tman_arg_t * args)
{
    char path[PATH_MAX + 1];
    sprintf(path, "%s/%s/.tman", taskdir, args->project);
    return MKDIR(path);
}
