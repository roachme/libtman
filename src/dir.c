...
static int _task_dbmkdir(char *base, tman_arg_t * args)
{
    ...
    sprintf(path, fmt, base, args->project, args->board, args->taskid);
    return MKDIR(path);
}
...
{
    char path[PATHSIZ + 1];
    sprintf(path, "%s/%s/%s/.tman", base, args->project, args->board);
    return MKDIR(path);
}
...
{
    char path[PATHSIZ + 1];
    sprintf(path, "%s/%s/.tman", base, args->project);
    return MKDIR(path);
}
...
{
    char path[PATHSIZ + 1];
    sprintf(path, "%s/%s/%s/%s", base, args->project, args->board,
            args->taskid);
...
int dir_task_move(char *base, tman_arg_t * src, tman_arg_t * dst)
{
    ...
    sprintf(old_path, "%s/%s/%s/%s", base, src->project, src->board,
            src->taskid);
    sprintf(new_path, "%s/%s/%s/%s", base, dst->project, dst->board,
            dst->taskid);
...
{
    char path[PATHSIZ + 1];
    sprintf(path, "%s/%s/%s", base, args->project, args->board);
...
{
    char path[PATHSIZ + 1];
    sprintf(path, "%s/%s/", base, args->project);
...
int dir_prj_rename(char *base, tman_arg_t * src, tman_arg_t * dst)
{
    ...
    sprintf(srcpath, "%s/%s", base, src->project);
    sprintf(dstpath, "%s/%s", base, dst->project);
